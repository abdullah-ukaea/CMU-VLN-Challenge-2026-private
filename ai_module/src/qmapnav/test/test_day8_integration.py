"""Two-view Day 8 colour fusion and relation-reevaluation regression."""
...
from test_object_map import _candidate
from test_object_map import _observation
...
def test_two_view_colour_stability_and_relation_geometry_update() -> None:
    ...
    chair_a = _candidate('chair_a', (1.0, 2.0, 0.5))
    chair_id = object_map.add_or_update(
        chair_a, _observation(chair_a, 'view_a', 1)
    )
    ...
    chair_b = _candidate('chair_b', (1.02, 2.01, 0.5))
    assert object_map.add_or_update(
        chair_b, _observation(chair_b, 'view_b', 2)
    ) == chair_id
